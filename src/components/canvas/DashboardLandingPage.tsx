import { Phone, Scissors, ShowerHead, PawPrint } from 'lucide-react';
import type { SiteConfig, DashboardFocus } from '../../types';
import TestimonialsSection from './sections/TestimonialsSection';
import ServicesPricingSection from './sections/ServicesPricingSection';
import ContactSection from './sections/ContactSection';

interface DashboardLandingPageProps {
  siteConfig: SiteConfig;
  focusedSection: DashboardFocus;
  onFocus: (section: DashboardFocus) => void;
}

// Returns a subtle animated focus ring via box-shadow (no layout impact)
function ring(key: 'headline' | 'subheadline', focused: DashboardFocus): React.CSSProperties {
  return {
    boxShadow: focused === key ? '0 0 0 2px rgba(124,58,237,0.55)' : '0 0 0 2px transparent',
    borderRadius: 8,
    cursor: 'pointer',
    transition: 'box-shadow 0.15s ease',
  };
}

export default function DashboardLandingPage({
  siteConfig,
  focusedSection,
  onFocus,
}: DashboardLandingPageProps) {
  const { colors } = siteConfig;

  const stop = (fn: () => void) => (e: React.MouseEvent) => {
    e.stopPropagation();
    fn();
  };

  return (
    <div
      className="min-h-full w-full"
      style={{
        backgroundColor: colors.canvasBg,
        transition: 'background-color 0.4s ease',
        fontFamily: colors.fontFamily,
      }}
      onClick={() => onFocus('background')}
    >
      {/* ── Navbar ── */}
      <div
        className="flex items-center justify-between px-5 py-3"
        style={{ backgroundColor: colors.cardBg, transition: 'background-color 0.4s ease' }}
        onClick={stop(() => onFocus(null))}
      >
        <div className="flex items-center gap-2">
          <PawPrint className="w-5 h-5" style={{ color: colors.accent }} />
          <span className="font-bold text-sm" style={{ color: colors.headline }}>
            {siteConfig.businessName}
          </span>
        </div>
        <button
          className="text-xs font-bold px-3.5 py-1.5 rounded-full"
          style={{ backgroundColor: colors.primaryBtn, color: colors.primaryBtnText }}
          onClick={stop(() => onFocus(null))}
        >
          Book Now
        </button>
      </div>

      {/* ── Hero ── */}
      <div className="px-5 pt-5 pb-4" onClick={(e) => e.stopPropagation()}>
        <div
          className="inline-block text-xs font-bold px-3 py-1 rounded-full mb-3"
          style={{ backgroundColor: `${colors.accent}22`, color: colors.accent }}
        >
          ⭐ Rated #1 Mobile Groomer
        </div>

        {/* Headline — tappable */}
        <h1
          className="text-[22px] font-extrabold leading-tight mb-2"
          style={{
            ...ring('headline', focusedSection),
            color: colors.headline,
            transition: 'color 0.4s ease, box-shadow 0.15s ease',
          }}
          onClick={stop(() => onFocus('headline'))}
        >
          {siteConfig.headline}
        </h1>

        {/* Subheadline — tappable */}
        <p
          className="text-sm leading-relaxed"
          style={{
            ...ring('subheadline', focusedSection),
            color: colors.body,
          }}
          onClick={stop(() => onFocus('subheadline'))}
        >
          {siteConfig.subheadline}
        </p>
      </div>

      {/* ── Dog image ── */}
      <div
        className="mx-5 rounded-2xl overflow-hidden mb-4"
        style={{
          height: '150px',
          backgroundColor: `${colors.accent}25`,
          transition: 'background-color 0.4s ease',
        }}
        onClick={stop(() => onFocus(null))}
      >
        <div className="w-full h-full flex flex-col items-center justify-center gap-1 select-none">
          <span className="text-6xl">🐶</span>
          <span className="text-xs font-semibold" style={{ color: colors.body }}>
            Fresh &amp; Fluffy
          </span>
        </div>
      </div>

      {/* ── CTA button ── */}
      <div className="px-5 mb-5" onClick={stop(() => onFocus(null))}>
        <button
          className="w-full py-4 rounded-2xl font-bold text-base"
          style={{
            backgroundColor: colors.primaryBtn,
            color: colors.primaryBtnText,
            boxShadow: `0 6px 24px ${colors.primaryBtn}55`,
            transition: 'background-color 0.4s ease',
          }}
        >
          {siteConfig.ctaText}
        </button>
      </div>

      {/* ── Services ── */}
      <div className="px-5 mb-4" onClick={stop(() => onFocus(null))}>
        <h2 className="text-base font-bold mb-3" style={{ color: colors.headline }}>
          Our Services
        </h2>
        <div className="flex flex-col gap-2">
          {(
            [
              { icon: ShowerHead, label: 'Bath & Blow-Dry', price: 'from $45' },
              { icon: Scissors, label: 'Full Groom & Trim', price: 'from $65' },
              { icon: Phone, label: 'Mobile Pickup', price: 'FREE' },
            ] as const
          ).map(({ icon: Icon, label, price }) => (
            <div
              key={label}
              className="flex items-center gap-3 p-3 rounded-xl"
              style={{ backgroundColor: colors.cardBg, transition: 'background-color 0.4s ease' }}
            >
              <div
                className="w-9 h-9 rounded-full flex items-center justify-center shrink-0"
                style={{ backgroundColor: `${colors.accent}22` }}
              >
                <Icon className="w-4 h-4" style={{ color: colors.accent }} />
              </div>
              <span className="text-sm font-medium flex-1" style={{ color: colors.headline }}>
                {label}
              </span>
              <span className="text-xs font-bold" style={{ color: colors.accent }}>
                {price}
              </span>
            </div>
          ))}
        </div>
      </div>

      {/* ── Extra Blocks ── */}
      {siteConfig.extraBlocks?.map((block) => {
        if (block.type === 'testimonials')
          return <TestimonialsSection key={block.id} colors={colors} />;
        if (block.type === 'services-pricing')
          return <ServicesPricingSection key={block.id} colors={colors} />;
        if (block.type === 'contact') return <ContactSection key={block.id} colors={colors} />;
        return null;
      })}

      {/* ── Footer ── */}
      <div className="px-5 pb-8 text-center">
        <p className="text-xs" style={{ color: `${colors.body}90` }}>
          📍 Serving your neighborhood • 7 days a week
        </p>
      </div>
    </div>
  );
}
