...
- For idea_thinking_board: read the user's prompt carefully — the INTENT changes everything.
  ...
  * FRAMEWORKS (optional fields, intent-dependent): for business/validate boards include "fiveWhys" — 3-5 entries drilling from the surface problem down to the root cause (each one plain sentence answering "why?" about the previous; the last entry IS the root cause); omit decisionMatrix. For DECISION and COMPARISON boards include "decisionMatrix" — 2-4 options, 3-5 criteria with weight 1-5 (how much that factor matters), and scores[optionId][criterionId] 1-10 scored honestly, not diplomatically; omit fiveWhys. For learning and brainstorm boards omit both.
